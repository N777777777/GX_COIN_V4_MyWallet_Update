-- جدول لتتبع عدد الإعلانات المشاهدة يومياً
CREATE TABLE public.daily_ad_views (
  ...
  UNIQUE(telegram_user_id, view_date)
);

-- تمكين RLS
ALTER TABLE public.daily_ad_views ENABLE ROW LEVEL SECURITY;

-- سياسات للوصول
CREATE POLICY "Users can view their own ad views" 
ON public.daily_ad_views 
FOR SELECT 
USING (true);
...
-- دالة للتحقق من عدد الإعلانات اليومية وإضافة عرض جديد
CREATE OR REPLACE FUNCTION public.handle_ad_view_and_check_qualification(
