...
ON public.partner_tasks 
FOR UPDATE 
USING (created_by IN (SELECT id FROM telegram_users WHERE telegram_id::text = auth.uid()::text));
...
FOR SELECT 
USING (telegram_user_id IN (SELECT id FROM telegram_users WHERE telegram_id::text = auth.uid()::text));
