-- إصلاح دالة get_user_mining_record لاستخدام أسماء الأعمدة الصحيحة
...
BEGIN
  ...
  RETURN json_build_object(
    ...
    'gcoin_v4_balance', COALESCE(user_record.bal_g4v7y, 0),
    ...
    'user_alpha_coins', COALESCE(user_record.bal_a6c3z, 0),
...
-- إصلاح دالة mine_gcoin لاستخدام أسماء الأعمدة الصحيحة
...
BEGIN
  ...
  -- التحقق من وجود عملات ألفا كافية (bal_a6c3z هو alpha_coins)
  IF COALESCE(user_record.bal_a6c3z, 0) < alpha_cost THEN
    RETURN json_build_object(
      'success', false,
  ...
  -- خصم عملات الألفا وإضافة G COIN V4
  UPDATE public.telegram_users 
  SET 
    bal_a6c3z = bal_a6c3z - alpha_cost,
    bal_g4v7y = COALESCE(bal_g4v7y, 0) + gcoin_reward,
    updated_at = NOW()
  WHERE id = p_telegram_user_id;
  ...
  RETURN json_build_object(
    ...
    'new_gcoin_balance', COALESCE(user_record.bal_g4v7y, 0) + gcoin_reward,
    'new_alpha_balance', user_record.bal_a6c3z - alpha_cost
  );
